...
func (c *PostgresClient) GetTables(schema string) (Result, error) {
	...
	data, err := executeSelectQuery(c.Db, "information_schema.tables WHERE table_schema = $1", schema)
	if err != nil {
		return result, err
	}
	result.Data = data
...
func (c *PostgresClient) GetTable(schema string, table string) (Result, error) {
	...
	info, err := executeSelectQuery(c.Db, "information_schema.columns WHERE table_schema = $1 AND table_name = $2", schema, table)
	if err != nil {
		return result, err
	}
	result.Info = info
